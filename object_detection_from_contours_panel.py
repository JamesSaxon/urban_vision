...
for nframe in tqdm(range(NFRAMES), desc = "Video"):
    ...
    for c in contours:
        ...
        x_mid = (x + w//2)*SCALE
        y_mid = (y + h//2)*SCALE
        grid_square = (x_mid//250, y_mid//250)
        if grid_square in areas_inferenced: continue

        areas_inferenced.add(grid_square)

        xmin = max(250*grid_square[0] - 50, 0)
        xmax = xmin + 400
        ymin = max(250*grid_square[1] - 50, 0)
        ymax = ymin + 400
        ...
        if view: cv2.rectangle(img, (xmin,ymin), (xmax,ymax), (255,255,0), 3)
...
cv2.waitKey(10)
tracker.write_out('tracker_output_contour_panels.csv')
